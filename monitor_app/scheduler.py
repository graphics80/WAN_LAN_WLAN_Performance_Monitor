...
def start_scheduler(client, config: AppConfig) -> BackgroundScheduler:
    ...
    if config.enable_speedtest:
        ...
        logging.info("Speedtests disabled via ENABLE_SPEEDTEST=false; skipping scheduling")

    if config.enable_download_tests:
        ...
        logging.info("Download tests disabled via ENABLE_DOWNLOAD_TESTS=false; skipping scheduling")

    if config.enable_http_tests:
        schedule_http_load_jobs(scheduler, client, config)
    else:
        logging.info("HTTP load tests disabled via ENABLE_HTTP_TESTS=false; skipping scheduling")
    scheduler.start()
    return scheduler
