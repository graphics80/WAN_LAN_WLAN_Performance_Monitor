...
from monitor_app.tasks import speedtest


def test_run_speedtest_for_interface(monkeypatch):
    monkeypatch.setattr(speedtest, "get_interface_ip", lambda iface: "1.2.3.4")

    def fake_run(cmd, capture_output, text):
        payload = '{"download": 1000000, "upload": 2000000, "ping": 5}'
        return SimpleNamespace(returncode=0, stdout=payload, stderr="")

    monkeypatch.setattr(speedtest.subprocess, "run", fake_run)
    metrics = speedtest.run_speedtest_for_interface("eth0")
...
def test_run_speedtests_writes(monkeypatch):
    ...
    monkeypatch.setattr(speedtest, "run_speedtest_for_interface", lambda iface: {"download_mbps": 1, "upload_mbps": 2, "ping_ms": 3})
    ...
    monkeypatch.setattr(speedtest, "write_metric", fake_write)
    speedtest.run_speedtests(None, cfg)
