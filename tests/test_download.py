...
from monitor_app.tasks import download


def test_download_file(monkeypatch, tmp_path: Path):
    monkeypatch.setattr(download, "get_interface_ip", lambda iface: "1.2.3.4")
    ...
    metrics = download.download_file("https://example.com/file", "eth0")
...
def test_run_download_tests_writes(monkeypatch):
    ...
    monkeypatch.setattr(download, "download_file", lambda url, iface: {"bandwidth_mbps": 10, "file_size_bytes": 100, "duration_seconds": 1})
    ...
    monkeypatch.setattr(download, "write_metric", fake_write)
    download.run_download_tests(None, cfg)
