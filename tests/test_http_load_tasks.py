...
from monitor_app.tasks import http_load_tasks
...
def test_run_http_load_job_writes(monkeypatch):
    ...
    monkeypatch.setattr(http_load_tasks, "run_http_load_for_target", lambda iface, url, c: sample_stats)
    calls = []

    def fake_write(client, config, measurement, tags, fields):
        calls.append((measurement, tags, fields))

    monkeypatch.setattr(http_load_tasks, "write_metric", fake_write)
    http_load_tasks.run_http_load_job("eth0", "https://example.com", None, cfg)
...
def test_schedule_http_load_jobs_offsets(monkeypatch, dummy_scheduler):
    ...
    # Keep jobs light: patch run_http_load_job placeholder
    monkeypatch.setattr(http_load_tasks, "run_http_load_job", lambda i, u, client, c: None)
    http_load_tasks.schedule_http_load_jobs(dummy_scheduler, None, cfg)
    # Expect 4 jobs (2 iface x 2 urls) with unique ids
    ...
    # Offsets should be spaced by interval/total_jobs (within small tolerance)
